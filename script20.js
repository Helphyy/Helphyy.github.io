...
var infoCases = null;
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      setInterval(intervalForCases,50)
    }
  };
  xhttp.open("GET", "https://donjonsansdragon.space/artPlaCaseGetM.php", false);
  xhttp.send();
...
if(window.innerWidth > window.innerHeight){
  ...
  for(var i = 0; i < listeTable.length;i++){
    listeTable[i].style.width = window.innerHeight/3-1 -  6+"px";
    listeTable[i].style.height = window.innerHeight/3-1 -  6+"px";
  }
}else{
  ...
  for(var i = 0; i < listeTable.length;i++){
    listeTable[i].style.width = window.innerWidth/3-1 -  6+"px";
    listeTable[i].style.height = window.innerWidth/3-1 -  6+"px";
  }
}
